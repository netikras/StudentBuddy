...
public abstract class LecturerApi {
    ...
    @RestEndpoint(url = "/discipline/id/{id}", method = HttpMethod.GET, dtoType = LecturerDto.class)
    public abstract List<LecturerDto> getByDiscipline(
            @MethodParam(type = Type.URL, name = "id") String disciplineId);


    @RestEndpoint(url = "/person/id/{id}", method = HttpMethod.GET, dtoType = LecturerDto.class)
    public abstract List<LecturerDto> getAllByPersonId(
            @MethodParam(type = Type.URL, name = "id") String personId);


    @RestEndpoint(url = "/search/degree/{degree}", method = HttpMethod.GET, dtoType = LecturerDto.class)
    public abstract List<LecturerDto> searchAllLecturersByDegree(
            @MethodParam(type = Type.URL, name = "degree") String degreeSubstring);

    @RestEndpoint(url = "/search/fname/{fname}", method = HttpMethod.GET, dtoType = LecturerDto.class)
    public abstract List<LecturerDto> searchByFirstName(
            @MethodParam(type = Type.URL, name = "fname") String fnameSubstring);


    @RestEndpoint(url = "/search/lname/{lname}", method = HttpMethod.GET, dtoType = LecturerDto.class)
